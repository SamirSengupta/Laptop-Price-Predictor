...
add_bg_from_local([r'background.jpg', r'background.jpg',r'background.jpg'])
...
# Heading of the Page
st.markdown("<h1 style='text-align: center;'>The Predictor</h1>", unsafe_allow_html=True)
st.markdown("<h5 style='text-align: center; '>A better laptop price predictor and recommender</h5>", unsafe_allow_html=True)

# All inputs:
# brand
brand_options = df['Company'].unique().tolist()
brand_options.insert(0, 'Select Brand')
company = st.selectbox('Brand', brand_options)

# type of laptop
type_options = df['TypeName'].unique().tolist()
type_options.insert(0, 'Select Type')
laptop_type = st.selectbox('Type', type_options)

# Ram
ram_options = [2, 4, 6, 8, 12, 16, 24, 32, 64]
ram_options.insert(0, 'Select RAM capacity')
ram = st.selectbox('RAM(in GB)', ram_options)

# weight
weight = st.number_input('Select Weight (kg)', format='%g', value=0)
...
os = st.selectbox('OS', ['Select OS'] + os_options)
...
if expand_sidebar:
    ...
    st.caption("<p class='caption-text'>The laptop price forecasts generated by this application rely on historical data and machine learning algorithms. It's worth noting that real prices may differ due to market conditions and other variables. We recommend users to view this information as a useful point of reference and supplement it with further research and expert guidance. Our app aims to aid users in making well-informed purchasing choices by offering valuable insights into laptop prices and providing recommendations.</p>", unsafe_allow_html=True)

st.text("")
col1, col2, col3 = st.columns(3)
with col1:
    predict_price_checked = st.button('Price Predict')
with col3:
    recommendation_checked = st.button('Recommend')
    ...
    if company == 'Select an option' or laptop_type == 'Select an option' or ram == 'Select an option' or \
        ...
        try:
            ...
            if hdd == 'Select an option':
                ...
                st.write("Please don't enter 0, mention some values")
...
if recommendation_checked:
    if company == 'Select an option' or laptop_type == 'Select an option' or ram == 'Select an option' or \
        ...
        st.error('Please select the specifications for the prediction.')
    else:
